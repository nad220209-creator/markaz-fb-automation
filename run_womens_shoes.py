...
SEARCH_QUERY = "Womens Casual Sneakers Khussa"
...
def get_single_product():
        ...
        if res.status_code == 200:
            ...
            for a in soup.find_all("a", href=True):
                ...
                if "/product/" in href:
                    ...
                    # STRICT WOMEN'S SHOES FILTER
                    has_shoes = any(kw in title_lower for kw in ["shoe", "shoes", "sneaker", "sneakers", "khussa", "sandal", "heel", "pumps", "footwear"])
                    has_women = any(kw in title_lower for kw in ["women", "womens", "ladies", "girl", "girls", "female"])
                    has_men = any(kw in title_lower for kw in ["men", "mens", "boy", "boys", "gents"])
                    
                    if len(title) > 3 and has_shoes and has_women and not has_men:
    ...
    if not fresh_candidates:
        if candidates:
            ...
            print("Using verified fallback women's shoes URL.")
            return FALLBACK_URL
    ...
    print(f"Selected Women's Shoe Product -> Title: '{selected['title']}' | URL: {selected['url']}")
    return selected["url"]
...
def main():
    ...
    try:
        ...
        print(f"ERROR in women's shoes test: {e}")
