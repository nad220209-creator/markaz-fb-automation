...
def get_single_product():
        ...
        if res.status_code == 200:
            ...
            for a in soup.find_all("a", href=True):
                ...
                if "/product/" in href:
                    ...
                    if len(title) > 3 and has_lawn and not has_forbidden:
    ...
    if not fresh_candidates:
        if candidates:
            ...
            print("Search yielded no lawn suit items. Using verified fallback lawn suit URL.")
            return FALLBACK_URL
    ...
    print(f"Selected Verified Lawn Suit Product -> Title: '{selected['title']}' | URL: {selected['url']}")
    return selected["url"]
